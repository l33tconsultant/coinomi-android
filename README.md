Pot Wallet (A Coinomi Clone)
===============

Our goal is to support every cryptocurrency with an active development team. Store the best cryptocurrency through a single app, without sacrificing security. Private keys are stored on your own device. You get a master key that can be memorized or stored on a piece of paper. Restoring the wallet from a single recovery phrase.
